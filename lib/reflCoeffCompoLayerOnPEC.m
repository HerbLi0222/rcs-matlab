function [RCperp, RCpar] = reflCoeffCompoLayerOnPEC(thri, phrii, alpha, beta, freq, matrlLine)
    % REFLCOEFFCOMPOLAYERONPEC Reflection coefficient for composite layer on PEC substrate.
    %
    %   Models a dielectric/magnetic layer backed by a perfect electric conductor.
    %   Uses the wave matrix (impedance-based) method.
    %
    %   Input:
    %       thri, phrii - incidence angles (radians)
    %       alpha, beta - facet orientation (radians)
    %       freq        - frequency (Hz)
    %       matrlLine   - material line with layers
    %   Output:
    ...
    for i = 1:length(layers)
        ...
        Beta(i) = 2 * pi / (wave / sqrt(real(erc) * real(urc)));
